...
\end{align}
where $\omega$ is the natural frequency and $\xi$ is the damping of the controlled inverter. An example would be a voltage source inverter connected to an LC filter, as the one shown in Figure~\ref{fig:VoltageSourceInverter}, controlled in abc frame by a cascaded voltage-current control system comprising P controllers.

We note that, with properly designed control, voltage source inverters are expected to behave as second order systems in closed loop. Assuming a system model of the form~(\ref{eq:InvMdl}) is thus not very restrictive.
...
\caption{Minimum of the damping $\alpha_i$ of the pole pairs of the closed loop system with $N=2$ PR controllers as a function of the PR controller gains $\lambda_1$ and $\lambda_3$. The labels on the contour lines indicate damping in degrees.}
